...
if($extractContigs_complete_byFile{$compatible_reference_file}{'*'})
{
	...
	my $extraction_command_unmapped = qq($samtools_bin view -H $BAM; $samtools_bin view $BAM '*' | awk '{if (\$3 == "*") print \$0}' | $samtools_bin view -Sb -o $target_extraction_unmapped -);
	print "Extract unmapped reads...\n";
	
	if(system($extraction_command_unmapped) != 0)
	{
		print "Warning: Extraction command failed, creating empty unmapped BAM file\n";
		my $create_empty_bam = qq($samtools_bin view -H $BAM | $samtools_bin view -S -b -o $target_extraction_unmapped -);
		system($create_empty_bam);
	}
...
else
{
	...
	my $MHC_PRG_2_bin = '/usr/local/app/miniconda3/opt/hla-la/bin/HLA-LA';
