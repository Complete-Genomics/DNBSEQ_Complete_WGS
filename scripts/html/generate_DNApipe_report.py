...
def getContentLang(contentName, lang):
    contentDict = {
        ...
        "ALIGNTABDES" : ["<p>&emsp;a. 比对结果基于低质量过滤及barcode拆分后的有效数据进行统计。</p><p>&emsp;b. 比对率和成对比对率分别表示正确和成对正确的比对到参考基因组的序列比例。</p><p>&emsp;c. 错配表示序列中与参考基因组不同的碱基信息。</p><p>&emsp;d. 重复序列表示来自PCR或其他因素导致的重复序列。</p><p>&emsp;e. 平均深度表示基因组平均覆盖深度。</p><p>&emsp;f. 覆盖度分别统计参考基因组中覆盖超过1层、4层、10层、20层的比例。</p><p>&emsp;g. 平均插入片段表示成对比对序列的平均片段长度。</p>", 
                         "<p>&emsp;a. The statistics results shown here are calculated based on clean data after low-quality filter and barcode-split filter.</p><p>&emsp;b. The mapping rate and paired mapping rate are the proportion of read which is mapped or paired-mapped to reference genome.</p><p>&emsp;c. The mismatch means the unmapped bases between read and reference genome.</p><p>&emsp;d. The duplication means duplicated reads introduced by PCR or other processes.</p><p>&emsp;e. The average sequencing depth is calculated by mapped reads.</p><p>&emsp;f. The coverage is calculated as proportion of reference genome covered by more than 1, 4, 10 or 20 folds reads.</p><p>&emsp;g. The mean insert size is the mean insert size of paired mapped reads.</p>"],
        ...
        "Clean depth"                    : ["有效深度(X)", "Clean depth(X)"],
        "Split barcode(G)"               : ["拆分后数据量(G)", "Split barcode(G)"],
        "Average sequencing depth"       : ["平均深度(X)", "Average sequencing depth(X)"],
        ...
        "barcodecount"                   : ["barcode数目", "barcodecount"],
        "ave_fragonbarcode"              : ["barcode平均含fragment数", "ave_fragonbarcode"],
        "ave_barcodereadcount"           : ["barcode平均深度", "ave_barcodereadcount"],
        "fragcount"                      : ["fragment数目", "fragcount"],
        "ave_fraglen"                    : ["fragment平均长度", "ave_fraglen"],
        "ave_fragreadcount"              : ["fragment平均深度", "ave_fragreadcount"],
        ...
        "SV TRA|BND"       : ["SV TRA|BND", "SV TRA|BND"],
...
def generate_html_report(path, path2, version, name='rem', output_path='E:/codezlims/rem/Result/sample1', lang="cn"):
    ...
    fastq_table     = path2 + "/" + name + ".fastqtable.xls"
    frag_table      = path2 + "/" + name + ".fragtable.xls"
    align_table     = path2 + "/" + name + ".aligntable.xls"
    variant_table   = path2 + "/" + name + ".varianttable.xls"
...
if __name__ == '__main__':
    ...
    generate_html_report(path, path2, version, name, outdir, "en")
    generate_html_report(path, path2, version, name, outdir, "cn")
