...
class error_result():
    def __init__(self, ref=None,switch_count=None,poss_sw=None,mismatch_count=None,poss_mm=None,flat_count=None,poss_flat=None,
                 phased_count=None,num_snps=None, diff_allele=None,maxblk_snps=None,    #added diff_allele=None,
        ...
        self.diff_allele  = create_dict(diff_allele, int)   ###diff_allele
    ...
    def __add__(self,other):
        ...
        new_err.diff_allele    = merge_dicts(self.diff_allele,    other.diff_allele)    ##diff_allele
...
N50:                {}
num snps max blk:   {}
            '''.format(self.get_switch_count(), self.get_poss_sw(), self.get_switch_rate(), 
                       self.get_mismatch_count(), self.get_poss_mm(), self.get_mismatch_rate(), 
                    self.get_flat_count(), self.get_poss_flat(), self.get_flat_error_rate(),
                    self.get_phased_count(), self.get_num_snps(), self.get_diff_allele(), self.get_blk_len(),
...
def error_rate_calc(t_blocklist, a_blocklist, vcf_file, indels=False, phase_set=None):
    ...
    for t_block in t_blocklist:
        ...
        for blk in a_blocklist:
            ...
            if phased_known >= 2:
                ...
                # print("valid block pair: reference {}; assemble {}; sw {}; mm {}".format(t_block[0][1], blk[0][1], poss_sw, poss_mm))
    ...
    total_error = error_result(ref=ref_name,
             ...
             phased_count=phased_count,num_snps=num_snps, diff_allele=different_alleles,
