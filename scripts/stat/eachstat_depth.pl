...
my ($basethres,$mapQthres,$total_chr,$help,$hg,$bam2depth,$non,$R,$name,$t);
GetOptions("q:i"=>\$basethres,"Q:i"=>\$mapQthres,"hg:s"=>\$hg,"h"=>\$help, "bd:s" => \$bam2depth,"n:s"=>\$non,"R:s"=> \$R, "s:s" => \$name, "t:i" => \$t);
...
if($hg =~ /hg19/i){
  open DEPTH,"$bam2depth depth -@ $t -a -b $non $bam | awk '{print \$3}'| " or die;
}
elsif($hg =~ /hs37d5/i){
  open DEPTH,"$bam2depth depth -@ $t -a -b $non $bam | awk '{print \$3}'| " or die;
