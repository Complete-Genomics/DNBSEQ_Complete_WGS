...
def parse_hapblock_file(hapblock_file,vcf_file,indels=False):
    ...
    return [b for b in blocklist if len(b) > 1]     #[[(var_idx, pos, GT1, GT2, ref, alt1, alt2),(),...],[second block],...]

def parse_vcf_phase(vcf_file, CHROM, indels = False):       #benchmark phased vcf
    ...
    with gzip.open(vcf_file, 'rt') as vcf:
        ...
        for line in vcf:
            ...
            phase_data = el[9]  # 1|1:142:0,77:0,45:212:1/1:.:PATMAT
            ...
            dat = el[9].split(':')  # 1|1:93:2,56:2,38:99:1/1:.:PATMAT
            genotype = dat[0]
    ...
    return [v for k,v in sorted(list(blocks.items())) if len(v) > 1]    #blocks: [PATMAT: [(var_idx, pos, 0, 1, ref, alt, 2nd_alt),(),...]]
...
def hapblock_vcf_error_rate(assembly_file, vcf_file, phased_vcf_file, indels, largest_blk_only=False): #hapblock, vcf, pvcf, indelsflg

    # parse and get stuff to compute error rates
    CHROM = get_ref_name(vcf_file)
    t_blocklist = parse_vcf_phase(phased_vcf_file, CHROM, indels)               #benchmark
    a_blocklist = parse_hapblock_file(assembly_file,vcf_file,indels)            #real 
    # compute error result object
    if largest_blk_only:
        largest_blk = []
