...
while(<IN3>)
{
    chomp;
    my @pair =split(/\t/,$_);
    $map{$pair[0]}=$pair[1] ; # 726_813_160     CGACACGGTTTGGGCC
}
close IN3;
...
open OUT,"| gzip > $ARGV[3]/sample_S1_L001_R1_001.fastq.gz" or die "failed to open sample_S1_L001_R1_001.fastq.gz for write";
open OUT2,"| gzip > $ARGV[3]/read-I1_si-TTCACGCG_lane-001-chunk-001.fastq.gz" or die "failed to open read-I1_si-TTCACGCG_lane-001-chunk-001.fastq.gz for write";
open OUT3,"| gzip > $ARGV[3]/sample_S1_L001_R2_001.fastq.gz" or die "failed to open sample_S1_L001_R1_001.fastq.gz for write";
# open OUT4, "> $ARGV[3]/fake10x_stlfr.map" or die "failed to open sample_S1_L001_R1_001.fastq.gz for write";
...
while(<IN1>)
{
    ...
    my @name=split(/\//, $name1[1]); #1335_550_1232
    if( ! exists($map{$name[0]} ) )  # 0_0_0
    {
        $S=<IN1>;
        $S=<IN1>;
        $S=<IN1>;
    ...
    $N++;
    $seq="\@ST-E0:0:SIMULATE:8:0:0:$N$ARGV[4]#$name[0]";
    ## 1th
    print OUT "$seq 1:N:0:NAAGTGCT\t$name[0]\n"; #OUT: read1
    ## 2th
    $S=<IN1>;
