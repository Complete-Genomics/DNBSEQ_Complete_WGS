...
include {
    ...
    bam                              } from "${params.MOD}/parseSample"
include {
    fq;
    fq1; 
    readLen;
...
workflow CWGS {
    ...
    if (params.stLFR_only) {
        ...
        if ( !params.frombam) {
            ...
            parse_sample_frombam(ch_input).reads.set {ch_bam}
        ...
        if (!params.frombam) {
            ...
            if (params.align_tool.contains("bwa")) {
                ...
                if (params.var_tool.contains("gatk")) {
                    ...
                    if (params.stLFR_only) { 
                        svpre(phaseBwaGatk.out.svpre).groupTuple().map { items ->
                            def id = items[0]
                            def paths = items[1].flatten()
                            return [id, paths]
                        }.set {ch_svsplit}   
